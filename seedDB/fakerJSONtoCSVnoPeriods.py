...
#open write stream
    with open(File_out_places, mode='wt', encoding="utf-8") as places:
        ...
        placesCSV.writerow(["place_id", "name", "formatted_address", "international_phone_number", "website", "url", "open_now", "lat", "lon"])
        with open(File_out_openHours, mode='wt', encoding="utf-8") as openHours:
            openHoursCSV = csv.writer(openHours)
            openHoursCSV.writerow(["place_id", "openday", "opentime", "closeday", "closetime", "weekday_text"])
            ...
            for documentRow in jsonFile:
                ...
                placesRow = [document['place_id'], document['name'], document['formatted_address'], document['international_phone_number'], document['website'], document['url'], document['opening_hours']['open_now'], document['geometry']['location']['lat'], document['geometry']['location']['lng']]
                ...
                for period in document['opening_hours']['periods']:
                    #add to places csv
                    periodRow = [document['place_id'], period['open']['day'], period['open']['time'], period['close']['day'], period['close']['time'], document['opening_hours']['weekday_text'][j]]
                    j += 1;
